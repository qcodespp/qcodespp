...
class Model_340_Curve(InstrumentChannel):
    ...
    def __init__(self, parent: 'Model_340', index: int) ->None:
...
class Model_340_Sensor(InstrumentChannel):
    """
    ...
    def __init__(self, parent: 'Model_340', name: str, inp: str) ->None:

        if inp not in ['A', 'B', 'C1','C2','C3','C4','D1','D2']:
        ...
        self.add_parameter(
            ...
            val_mapping={"Special":0,
                "Silicon diode": 1,
                "GaAlAs diode": 2,
                "100 Ohm platinum/250": 3,
                "100 Ohm platinum/500": 4,
                "1000 Ohm platinum": 5,
                "Rhodium Iron": 6,
                "Carbon-Glass":7,
                "Cernox":8,
        ...
        Group(
            ...
            set_cmd=f"INTYPE {self._input}, {{type}}, {{units}}",
        ...
        self.add_parameter(
            ...
            get_parser=int,
            vals=Numbers(min_value=1, max_value=60)
        )
    ...
    @property
    def curve(self) ->Model_340_Curve:
        parent = cast(Model_340, self.parent)
        return Model_340_Curve(parent,  self.curve_index())


class Model_340_Heater(InstrumentChannel):
    """
    ...
    """
    def __init__(self, parent: 'Model_340', name: str, loop: int) ->None:
        ...
        Group(
            [self.input_channel, self.unit, self.enabled, self.powerup_enable],
            set_cmd=f"CSET {self._loop}, {{input_channel}}, {{unit}}, "
                    f"{{enabled}}, {{powerup_enable}}",
        ...
        valid_output_ranges = Enum(0, 1, 2, 3, 4, 5)
...
class Model_340(VisaInstrument):
    ...
    def __init__(self, name: str, address: str, channel_names=['A', 'B', 'C1','C2','C3','C4','D1','D2'], **kwargs) -> None:
        ...
        heaters = ChannelList(
            self, "heater", Model_340_Heater, snapshotable=False)

        for loop in [1, 2]:
            heater = Model_340_Heater(self, 'heater_{}'.format(loop), loop)
        ...
        curves = ChannelList(
            self, "curve", Model_340_Curve, snapshotable=False
        )

        for curve_index in range(1, 35):
            curve = Model_340_Curve(self, curve_index)
            curves.append(curve)
    ...
    def upload_curve(
        ...
        sensor_unit = Model_340_Curve.validate_datadict(data_dict)
